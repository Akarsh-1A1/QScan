...
class CBOMGenerator:
    ...
    def generate(self, domain: str, scan_results: List[Dict], timestamp: str = None) -> Dict:
        """
        Generate a complete CBOM document.

        Args:
            domain: Target organization domain
            scan_results: List of classified scan results
            timestamp: Scan timestamp string

        Returns:
            Complete CBOM document as a dictionary
        """
    ...
    def _build_asset_entry(self, result: Dict, index: int) -> Dict:
        ...
        entry = {
            ...
            "quantum_assessment": {
                ...
                "threat_assessment": result.get("quantum_threat_assessment", {}),
            },
            "recommendations": result.get("pqc_recommendations", []),
        }
    ...
    def _generate_summary(self, results: List[Dict]) -> Dict:
        ...
        for r in results:
            ...
            if status in pqc_counts:
                pqc_counts[status] += 1
            if r.get("cipher_analysis", {}).get("forward_secrecy"):
                with_fs += 1
    ...
    def _generate_risk_matrix(self, results: List[Dict]) -> List[Dict]:
        ...
        for r in results:
            matrix.append({
                ...
                "migration_deadline": r.get("quantum_threat_assessment", {}).get("migration_deadline"),
                "urgency": r.get("quantum_threat_assessment", {}).get("urgency"),
            })
    ...
    def _generate_migration_plan(self, results: List[Dict]) -> Dict:
        ...
        for r in results:
            ...
            port = r.get("port", 0)
            for rec in r.get("pqc_recommendations", []):
